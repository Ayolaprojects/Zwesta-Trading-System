...
"%PS_EXE%" -NoProfile -ExecutionPolicy Bypass -Command "$names = @('postgresql-x64-18','postgresql-x64-17'); $svc = Get-Service -Name $names -ErrorAction SilentlyContinue | Select-Object -First 1; if (-not $svc) { Write-Error 'No PostgreSQL Windows service found.'; exit 1 }; if ($svc.Status -ne 'Running') { Start-Service -Name $svc.Name; Start-Sleep -Seconds 3; $svc = Get-Service -Name $svc.Name }; if ($svc.Status -ne 'Running') { Write-Error ('PostgreSQL service failed to start: ' + $svc.Name); exit 1 }; Write-Host ('PostgreSQL service ready: ' + $svc.Name)"
